...
// Time-stamp: <2024-09-14 23:11:45>
...
func ListTasks(db *sql.DB, userID int64, page int64, limit int64) ([]Task, error) {
	...
	rows, err := db.Query(`
      SELECT id, user_id, title, state, due_at, created_at, updated_at, position
        FROM task
        WHERE user_id = ?
...
func FetchTask(db *sql.DB, userID int64, ID int64) (Task, error) {
	...
	err := db.QueryRow(
		"SELECT id, user_id, title, state, due_at, created_at, updated_at, position FROM task WHERE id = ? AND user_id = ?",
		ID,
		userID,
	).Scan(
		&t.ID,
...
func CreateTask(db *sql.DB, userID int64, t Task) (Task, error) {
	...
	stmt, err := db.Prepare(`INSERT INTO task (user_id, title, state, due_at, position)
      SELECT ?, ?, 'todo', datetime('now', '+1 day', 'start of day'), COALESCE(MAX(position), 0) + 1 FROM task
      RETURNING id, user_id, title, state, due_at, created_at, updated_at, position`)
